...
def temporal_closein_controls(df):
    ...
    print(f"\n{'='*70}")
    print("CLOSE-IN TEMPORAL DYNAMICS: Daily Windows (1-7 days, CA Tracts)")
    print(f"{'='*70}")
    ...
    # Load raw daily smoke data (smoke days only)
    print("  Loading raw daily smoke data (smoke days only)...")
    if not os.path.exists(SMOKE_FILE):
        print(f"  ERROR: Smoke file not found: {SMOKE_FILE}")
        return
    ...
    # Smoke file has only smoke-day rows: divide by 1 calendar day
    bin_vars = []
    for yr, edate_str in pres_dates.items():
        edate = pd.Timestamp(edate_str)
        ...
        for d in range(n_days):
            ...
            bin_sum = w.groupby("geoid")["smoke_pm25"].sum()
            bin_mean = (bin_sum / 1).rename(f"mean_day_{d}")
    ...
    if all(t[0] in all_treat_results for t in treat_configs):
        ...
        fig.suptitle("Close-In Temporal Dynamics (CA Tracts)", fontsize=14,
                     fontweight="bold", y=1.01)
        plt.tight_layout()
        fig_path = os.path.join(FIG_DIR, "ca_temporal_closein_daily.png")
        fig.savefig(fig_path, dpi=150, bbox_inches="tight")
        plt.close()
...
def main():
    ...
    # Close-in daily temporal dynamics (1-7 days)
    if pres_df is not None:
        temporal_closein_controls(pres_df)
